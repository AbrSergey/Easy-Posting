/**
 * This programs task it to create HTML code from the document that the user has created.
...
const { SYNTAX, modes } = require('./SYNTAX');

const createHTML = require('./dist/createHTML');
const setTag = require('./setters/setTag');
const setClass = require('./setters/setClass');
const checkMode = require('./checkers/checkMode');
const syntaxMispell = require('./feedback/syntaxMispell');

/*  FUNCTIONS FOR SETTING COLOR ON FEEDBACK IN TERMINAL */
const { error, warning, terminal, underline } = require('./feedback/terminal');
...
/**
 * The core principle of this code is that we set our whole code into different modes
 ...
 * previous section, while keep looping through the content of the document knowing
  ...
  set(target, key, value) {
    ...
    if (sectionContent.length !== 0) {
      ...
      for (const keys in MODES) {
        if (MODES[keys] === true) {
          tag = setTag(keys);
          className = setClass(keys);

          inMode = true;

          // Don't bother checking the rest, because there is only one mode active at a time.
          break;
        }
      }
      ...
      // Reset title tag in case it was changed
      SYNTAX.title[2] = 'h2';
...
// Creating a proxy for our object so we can detect when modes are changed
const MODES = new Proxy(modes, handler);
...
function setMode(wantedMode) {
  ...
  if (MODES.hasOwnProperty(wantedMode)) {
    ...
    throw Error(error(`${wantedMode} is not a valid mode`));
  }
}
 ...
 *
 * @param {__dirname/Directory} directory - Should be __dirnamr (or any other correct path) to the root-folder of the project
 * @param {directory} sourcefolder - The folder where are all the post-files are
 * @param {File} postfile - The file we want to create a post for
 */
...
async function createPost(directory, sourcefolder, postFile) {
  ...
  for await (let line of rl) {
     ...
     * If it does, we fire the syntaxMispell function and tells the user that "Hey, you wrote Title - 2, but you probably meant to write # Title -2"
     */
    ...
    if (inMode) {
      ...
      // Check if line includes an <code> tag, so that we can set the correct flow-setting
      if (line.includes('<code')) {
        ...
        for (let i = 0; i < lineArr.length; i++) {
          if (lineArr[i].includes('<code')) {
            if (
              lineArr[i + 1] !== undefined &&
              lineArr[i + 1].includes('flow')
            ) {
              ...
              // Here we have to replace the whole tag, because it's not required to have a space
              // between the tag and the content within (<code>myFunc()</code>), so it's not 100% sure that it got seperated in the split.
